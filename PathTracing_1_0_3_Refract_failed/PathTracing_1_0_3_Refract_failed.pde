...
void setup(){
  ...
  BxDF glass = new Glass(new Color(1, 1, 1), 1);
  ...
  path_tracer.primitives.add(new Sphere(new PVector(10, -13, 20).mult(worldScale), 4*worldScale, glossy));
  path_tracer.primitives.add(new Sphere(new PVector(-12, 0, 10).mult(worldScale), 8*worldScale, glass));
...
void draw(){
  ...
  cam.render();
  path_tracer.primitives.get(1).origin = new PVector((float)(mouseX-width/2)/30, (float)(height/2 - mouseY)/30, 20);
  if(pixelSize == 1){
    pixelSize = 20;
