class Camera{
  ...
  Camera(PVector eye, PVector focal, float view_dist, PVector up, int wid, int hei, int spp){
    this.eye = eye;
    this.focal = focal;
  ...
  void render(PathTraceIntegrator integrator){
    Ray ray = new Ray();
    ray.o = eye;
    for (float x = 0; x < wid; x++){
      for (float y = 0; y < hei; y++){
        ...
        rect(x*1, height-y*1-1, 1, 1);//it will be replace
      }
      println(x/wid*100+"%");
    }
  }
